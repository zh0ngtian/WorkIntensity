...
热力图每天的工时数字下方显示预计下班时间，⁺¹ 表示次日，* 表示暂定；悬浮详情也会显示该时间及「暂定」状态。

* 在任意 30 分钟滑动窗口内，累计至少 15 分钟活跃量（25 个不同的 36 秒时间块），才算持续活动，包含输入与会议活动。
* 预计下班时间取最后一个达标窗口内最后活跃块的起点，按分钟显示。较晚的零星操作若未使窗口达标，不会推迟下班时间；晚间再次持续工作并达标，则更新预计下班时间。
* 从该活跃块结束起满 30 分钟、没有新的达标活动，才确认时间；此前标为暂定，确认期不计入预计下班时间。状态按打开 Plot 时的记录计算，重新打开 Plot 可更新。
* 有活动但未达到持续活动阈值时，不显示下班时间，悬浮详情显示「无持续活动」；没有活动记录时显示「暂无记录」。该判定只影响预计下班时间，每日活跃工时仍累计全部活跃块。
